    ...
    >
      ...
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="min-w-0">
          ...
          <p className="text-gray-500 text-xs mt-0.5 line-clamp-1">{group.course?.title}</p>
        </div>
      </div>

      <p className="text-gray-500 text-xs mb-4 line-clamp-2">{group.description}</p>
      ...
      <div className="flex items-center justify-between text-xs text-gray-600 mb-4">
        <span className="flex items-center gap-1">
          <Users className="w-3 h-3" /> {group.members}/{group.maxMembers} members
        </span>
        <span>Active {timeAgo(group.lastActivity)}</span>
      </div>
      ...
      <button
        onClick={group.isMember ? onOpen : onJoin}
        ...
        disabled={spotsLeft === 0 && !group.isMember}
      >
        {group.isMember ? "Open Group" : spotsLeft === 0 ? "Full" : "Join Group"}
      </button>
    </motion.div>
  );
}
