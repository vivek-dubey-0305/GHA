...
import { authClient } from '../../utils/api.utils.js';
...
export const register = createAsyncThunk(
  'auth/register',
  async ({ firstName, lastName, email, password, confirmPassword, specialization, bio }, { rejectWithValue }) => {
    try {
      const payload = {
        firstName,
        lastName,
...
export const initializeAuth = createAsyncThunk(
  ...
  async (_, { dispatch, rejectWithValue }) => {
    try {
      ...
      // Use authClient which has withCredentials: true configured globally
      console.log("====================")
      const response = await authClient.post(`/refresh-token`, {});
...
export const selectInstructor = (state) => state.auth.instructor;
