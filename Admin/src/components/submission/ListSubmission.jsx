...
export function ListSubmission({ submissions, pagination, onSubmissionClick, onPageChange, searchTerm, onSearchChange }) {
  const dispatch = useDispatch();
  const deleteLoading = useSelector(selectDeleteSubmissionLoading);
  ...
  const handleConfirmDelete = async () => {
    if (submissionToDelete) {
      try {
        await dispatch(deleteSubmission(submissionToDelete._id)).unwrap();
      } catch (error) {
  ...
  return (
    <div className="flex-1 p-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400">Manage assignment submissions and grading</p>
      </div>
      ...
      <div className="mb-6 relative">
        ...
        <input
          type="text"
          placeholder="Search by assignment, user or status..."
          value={searchTerm}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-400">Assignment</th>
                ...
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-400">Submitted At</th>
              ...
              {filteredSubmissions.map((submission) => (
                ...
                >
                  <td className="px-6 py-4">
                    <span className="text-xs font-mono text-gray-300">{submission.assignmentId}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-xs font-mono text-gray-500">{submission.userId}</span>
                  </td>
                  <td className="px-6 py-4">
                    <Badge
                      variant="default"
                  ...
                  <td className="px-6 py-4 text-gray-300">{submission.score ?? '—'}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      >
                        {deleteLoading && submissionToDelete?._id === submission._id ? (
      ...
      <div className="flex items-center justify-between mt-6">
        <p className="text-sm text-gray-400">
          Showing {filteredSubmissions.length} of {pagination?.totalItems ?? submissions.length} submissions
